...
if (commander.tsc && typeCheckHelpers_1.isValidString(commander.tsc)) {
    ...
    debugTools_1.debugLog("Compiler command is true, so now searching for tsc executable");
...
debugTools_1.debugLog("Checking if global tslint rules is given", commander.lint);
if (commander.lint && typeCheckHelpers_1.isValidString(commander.lint)) {
    debugTools_1.debugLog("Global tslint rules set to", commander.lint);
    mtscConfig.tslint = commander.lint;
}
else if (commander.lint && typeCheckHelpers_1.isValidBoolean(commander.lint)) {
    debugTools_1.debugLog("Tslint rules is true");
    mtscConfig.tslint = commander.lint;
}
else if (commander.lint) {
    debugTools_1.debugLog("Invalid lint option given", commander.lint);
